...
const AddMemberBlock = ({ groupId, getMembers }) => {
    ...
    const teachers = useFetch({ method: "GET", url: `/api/settings/${groupId}`, get_autoFetch: true })
    const changeOwner = useFetch({ method: "PATCH", url: `/api/settings/${groupId}` })
    ...
    const handleAddMember = async () => {
        ...
        const { data, error } = await changeOwner.dispatch({ members: updatedTeacher })
        if (error) {
            console.log(error)
            return
        }
        ...
        <>
            <Modal open={modalOpen} onClose={onModalClose} >
                <Layout.Col className="gap-4 p-4 h-screen lg:min-w-[500px]">
                    <CustomComboBox placeholder="Select Teacher" list={availableTeachers().map(member => ({ value: member.email, displayValue: member.name }))} onChange={onOptionChange} value={currentValue} />
                    <Button className="btn-primary" loading={changeOwner.loading} onClick={handleAddMember}>Change</Button>
                </Layout.Col>
            </Modal>
            <Button className="btn-primary" loading={changeOwner.loading} onClick={onModalClose} >Change Ownership</Button>
        </>
    )
